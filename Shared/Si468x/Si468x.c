...
#define JSB_PROP_AUDIO_MUTE   0x0301
...
void si468x_power_up()
{
	uint8_t args[] =
	{
			...
			0x7F,	// IBIAS JSB: Check this! Was 7F. Works: 00 10 20 40 7F
			0x00,	// XTAL_FREQ
			0xF8,	// XTAL_FREQ
			0x24,	// XTAL_FREQ
			0x01,	// XTAL_FREQ ?! JSB: Was 01. Works: 01. Doesn't work: 02.
			0x3F,	// CTUN
			0x10,
			0x00,
			0x00,
			0x00,
			0x7F,	// IBIAS_RUN JSB: Check this! Was 7F. Works: 00 10 20 40 7F
			0x00,
			0x00
	};
...
{
  uint32_t StartAddress; // JSB added.

  switch(mode)
  {
    case Si468x_MODE_FM:
      StartAddress = 0x00006000; // JSB: Hardwired.
      break;
...
// JSB: This must be called in response to chip interrupt request.
//      E.g.
...
void JSB_si468x_set_mute(uint8_t Value)
{
  si468x_set_property(JSB_PROP_AUDIO_MUTE, Value ? 0x3 : 0x0);
}
