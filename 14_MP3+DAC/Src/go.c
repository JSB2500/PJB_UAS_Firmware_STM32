...
#define AudioData_MaxNumSamples (64 * 576) /* MP3 frame num bytes is 576 */
static AudioData_Sample_t AudioData[AudioData_MaxNumSamples];
static uint32_t AudioData_NumSamples = 0;

#define AudioDMA_CircularBuffer_NumSamplesPerHalf 16
...
static uint32_t AudioData_PlaySampleIndex = 0;
...
static short *MP3Decoder_pOutputSamples = NULL;
...
void HAL_SAI_TxHalfCpltCallback(SAI_HandleTypeDef *hsai)
{
  ...
  {
    if ((AudioData_PlaySampleIndex + AudioDMA_CircularBuffer_NumSamplesPerHalf) > AudioData_NumSamples)
      AudioData_PlaySampleIndex = 0;
    memcpy(&AudioDMA_CircularBuffer_Samples[0], &AudioData[AudioData_PlaySampleIndex], AudioDMA_CircularBuffer_NumSamplesPerHalf * sizeof(AudioData_Sample_t));
    AudioData_PlaySampleIndex += AudioDMA_CircularBuffer_NumSamplesPerHalf;
  }
}
...
void HAL_SAI_TxCpltCallback(SAI_HandleTypeDef *hsai)
{
  if (hsai == &hsai_BlockA2)
  {
    if ((AudioData_PlaySampleIndex + AudioDMA_CircularBuffer_NumSamplesPerHalf) > AudioData_NumSamples)
      AudioData_PlaySampleIndex = 0;
    memcpy(&AudioDMA_CircularBuffer_Samples[AudioDMA_CircularBuffer_NumSamplesPerHalf], &AudioData[AudioData_PlaySampleIndex], AudioDMA_CircularBuffer_NumSamplesPerHalf * sizeof(AudioData_Sample_t));
    AudioData_PlaySampleIndex += AudioDMA_CircularBuffer_NumSamplesPerHalf;
  }
}
...
void Go()
{
  ...
  if (!MP3Decoder_pMP3Info)
    Error_Handler();
  MP3Decoder_pOutputSamples = (short*)malloc(2 * AudioData_MaxNumSamples * sizeof(short));
  ...
  uint32_t NumSamplesDecoded = SpiritMP3Decode(MP3Decoder_pInstance, (short *)MP3Decoder_pOutputSamples, AudioData_MaxNumSamples, MP3Decoder_pMP3Info);
  //
  short *pMP3OutputSamples = MP3Decoder_pOutputSamples;
  AudioData_Sample_t *pAudioData = AudioData;
  for (uint32_t SampleIndex = 0; SampleIndex < NumSamplesDecoded; ++SampleIndex)
  {
    ...
    pAudioData->Left = Left16 << 16;
    pAudioData->Right = Right16 << 16;
    ++pAudioData;
  }
  AudioData_NumSamples = NumSamplesDecoded;
